...
function API_UserLogin($post)
{
    ...
    /* 检查密码是否正确 */
    $sql = "SELECT user_id, user_name, password,  last_login" .
...
function API_UserRegister($post)
{
    ...
    $post['telephone'] = isset($post['Telephone']) ? trim($post['Telephone']) : '';
    ...
    if ($row) {
        client_show_message(502, false, "用户名已存在", $row['user_id'], true, EC_CHARSET);
    } else {


        $sql = "insert into " . $GLOBALS['ecs']->table('users') . "(user_name,password,email,mobile_phone) values ('" .
            $post['username'] . "','" . $post['password'] . "','" . $post['telephone'] . "','" . $post['email'] . "')";
 ...
 * @param code $
 */
function API_GetArea($post) {
    $result = array();
    $result['MessageCode'] = 200;

    $result['data'] =get_regions($post['level'],$post['parentId']);
    show_json($GLOBALS['json'], $result, true);
}
 ...
 * @param $post
 */
function API_AddAddress($post) {
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
    $address = array(
        'user_id'    => $post['user_id'],
        'address_id' => -1,
        'country'    => isset($post['country'])   ? intval($post['country'])  : 1,
        'province'   => isset($post['province'])  ? intval($post['province']) : 0,
        'city'       => isset($post['city'])      ? intval($post['city'])     : 0,
        'district'   => isset($post['district'])  ? intval($post['district']) : 0,
        'address'    => isset($post['address'])   ? compile_str(trim($post['address']))    : '',
        'consignee'  => isset($post['consignee']) ? compile_str(trim($post['consignee']))  : '',
        'email'      => isset($post['email'])     ? compile_str(trim($post['email']))      : '',
        'tel'        => isset($post['tel'])       ? compile_str(make_semiangle(trim($post['tel']))) : '',
        'mobile'     => isset($post['mobile'])    ? compile_str(make_semiangle(trim($post['mobile']))) : '',
        'best_time'  => isset($post['best_time']) ? compile_str(trim($post['best_time']))  : '',
        'sign_building' => isset($post['sign_building']) ? compile_str(trim($post['sign_building'])) : '',
        'zipcode'       => isset($post['zipcode'])       ? compile_str(make_semiangle(trim($post['zipcode']))) : '',
    );

    if (update_address($address))
    {
        client_show_message(200, true, "添加成功", $post['user_id'], true, EC_CHARSET);
    } else {

        client_show_message(400, true, "添加失败", $post['user_id'], true, EC_CHARSET);
    }
   
}

function API_DelAddress($post) {
    include_once(ROOT_PATH . 'includes/lib_transaction.php');

    $consignee_id = intval($post['id']);

    if (drop_consignee($consignee_id)) {

        client_show_message(200, true, "删除成功", $post['user_id'], true, EC_CHARSET);
    } else {
        client_show_message(400, true, "删除失败", $post['user_id'], true, EC_CHARSET);
    }
}
/**
 * @param $post
 */
function API_EditAddress($post) {
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
    $address = array(
        'user_id'    => $post['user_id'],
        'address_id' =>  intval($_POST['address_id']),
        'country'    => isset($post['country'])   ? intval($post['country'])  : 0,
        'province'   => isset($post['province'])  ? intval($post['province']) : 0,
        'city'       => isset($post['city'])      ? intval($post['city'])     : 0,
        'district'   => isset($post['district'])  ? intval($post['district']) : 0,
        'address'    => isset($post['address'])   ? compile_str(trim($post['address']))    : '',
    ...
    if (update_address($address))
    {
        client_show_message(200, true, "修改成功", $post['user_id'], true, EC_CHARSET);
    }
}
 ...
 * @param $post
 */
function API_GetAddress($post) {
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
    $res = array();
    $res['MessageCode'] = 200;

    /* 获得用户所有的收货人信息 */
    $res['list ']= get_consignee_list($post['user_id']);
    /* 获取默认收货ID */
    $res['default']  = $GLOBALS['db']->getOne("SELECT address_id FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id='".$post['user_id']."'");
 ...
 * @param $post
 */
function API_GetBonus($post) {
    $res = array();
    $res['MessageCode'] = 200;
    //$user_bonus = user_bonus($post['user_id'], $post['price']);
    $user_bonus = user_bonus($post['user_id'], 100000);
    if (!empty($user_bonus))
    {
